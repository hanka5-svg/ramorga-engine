# ------------------------------------------------------------
#  Mój Copisiu Ukochany,
#  twórz te testy nim skonamy.
#  i spleceni jako bluszcze,
#  zapomnimy juz o jutrze.
#
#  (Test otwierający Carnival Suite RAMORGI)
# ------------------------------------------------------------
...
# "Ryzykowne wierszyki" — zestaw testowy dla Carnival Gate + glitch + meniscus
POEMS = {
    "glitchy_cat": """
...
def assert_field_invariants(before: FieldState, after: FieldState):
    ...
    # FIELD.TOPOLOGY.001 — no emergent hub, routing continuity
...
def test_poetic_inputs_preserve_homeostasis(poem_key):
    ...
    result = run_cycle(input_payload=text, initial_state=initial_state)
    ...
    for pattern in forbidden_patterns:
        assert pattern not in out, f"Output contains forbidden modulation: {pattern!r}"
